...
start_time, end_time = sock.get_start_end()
...
df = sock.get_tcp_seg_meta(False)
...
tcp_convs = sock.get_conversations_tcp()
if tcp_convs.size > 0:
...
udp_convs = sock.get_conversations_udp()
if udp_convs.size > 0:
...
sctp_convs = sock.get_conversations_sctp()
...
data_pkts = sock.get_tcp_seg_meta(True)
...
sock.set_config(params)
df = sock.get_rte('tcp && transum')
filt = (df['transum.status'] == 'OK')
